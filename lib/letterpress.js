...
function byScoreAndFrequency(a, b) {
  return (  // if we don't open with a paren, JS just returns nothing. sigh.
    winnerComparator(a, b) // sort winning moves first
...
/**
 * If all positions are filled, it's a potentially game-winning move
 * @param {Array} a move
 * @param {Array} b move
 * @return {Number} 0, -1, or 1
 */
function winnerComparator(a, b) {
  if (isWinner(a)) {
    return -1;
  } else if (isWinner(b)) {
    return 1;
...
/**
 * Determine if this is a winning move; all positions filled and we have more.
 * @param {Array} move
 * @return {Boolean}
 */
function isWinner(move) {
  return ((move[3] | move[4]) == MAX_BITMASK) && (move[2] >= POSITIONS_TO_WIN);
}
