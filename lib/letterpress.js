...
var getMovesForBoard = memoize(function(board, minFrequency, oursBitMask, theirsBitMask) {
  ...
  wordStructs.forEach(function(w) {
    ...
    flatCombos.forEach(function(positions) {
      var bitMask = getBitMaskForPositions(positions) | oursBitMask;
      var protectedBitMask = lpBitmask.getProtectedBitMask(bitMask);

      // and let's add this to the moves!
      moves.push([countBits(protectedBitMask), countBits(bitMask), bitMask, w]);
    });
  ...
  return moves
    ...
    // throw out moves that are similar to each other
 ...
 * A move is an array of [ protectedCount, bitCount, bitMask, [word, canonicalizedWord, frequency] ]
...
function byScoreAndFrequency(a, b) {
  return (b[0] - a[0]) || (b[1] - a[1]) || (a[3][2] - b[3][2]);
}
